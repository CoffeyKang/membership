            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gradient-to-r from-blue-50 to-indigo-50">
                    <tr>
                        ...
                        <th class="hidden md:table-cell px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">{{ __('messages.unpaid_amount') }}</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">{{ __('messages.today_amount') }}</th>
                        <th class="hidden md:table-cell px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">{{ __('messages.number_of_working_days') }}</th>
                        <th class="hidden md:table-cell px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">{{ __('messages.number_of_dayoffs') }}</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach($staff as $member)
                        <tr class="hover:bg-blue-50 transition-colors duration-150 cursor-pointer {{ $member->is_left ? 'bg-red-50 hover:bg-red-100' : '' }}">
                            ...
                            <td class="hidden md:table-cell px-6 py-4 whitespace-nowrap text-sm text-gray-700 font-semibold">$ {{ $member->unpaidAmount }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-green-600 font-semibold">$ {{ $member->today_amount }}</td>
                            <td class="hidden md:table-cell px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $member->numberOfWorkingDays }}</td>
                            <td class="hidden md:table-cell px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $member->numberOfDayoffs }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <a 
                                    href="{{ route('staff.show', $member->id) }}" 
                                    class="hidden md:inline text-indigo-600 hover:text-indigo-900 hover:underline mr-3">{{ __('messages.edit') }}</a>
                                <a 
                                    href="{{ route('staff.management.transactions', $member->id) }}" 
                                    class="hidden md:inline text-blue-600 hover:text-blue-900 hover:underline mr-3"
                                    wire:navigate
