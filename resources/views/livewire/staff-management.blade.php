        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Monthly Minimum Sales Amount</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @foreach($staff as $member)
                    <tr class="hover:bg-gray-100 cursor-pointer {{ $member->is_left ? 'bg-red-100 hover:bg-red-200' : '' }}">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $member->base_salary }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $member->monthly_minimum_sales_amount }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $member->commission_rate }}</td>
                ...
                @endforeach
                <tr>
                    <td colspan="5" class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
